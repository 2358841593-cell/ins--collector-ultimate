...
> 历史方案说明：账号/API 细节已被当前 browser-only 四阶段主线取代，执行以
> [`PIPELINE_SPEC.md`](PIPELINE_SPEC.md) 和 [`RUNBOOK.md`](RUNBOOK.md) 为准。
> 2026-07-28 补充：Storefront 是通用电商入口，不限 Amazon；展示型预估报价按
> “先排置顶、再取最近 10 条非置顶 Reels 均播 × CPM $35–$40 /1000”计算，并与实际
> `paid_cpm`、评分和路由隔离。
...
改为**登录态浏览器会话采集**：页面字段以渲染页读取为主；报价播放指标是明确例外，
由同一会话调用 Instagram 同源 media info，并只用 `ig_play_count` 计价。配合：
...
| 近帖 caption/赞/评论数/是否视频/置顶 | IG | 渲染帖子页 DOM（逐帖，拟人） |
| 赞助占比/导购占比/成分设备词/organic/Reels·Static ER | 派生 | 从近帖算 |
| Storefront 三态 + 类型/链接 | 浏览器穿透 | 识别 Amazon/LTK/ShopMy/自营店/购物聚合；确认无也继续，未知 Review |
...
| 展示型预估报价 | IG 同源 media info 派生，Modash 仅 fallback | 登录态浏览器会话逐条读取；先排置顶再取最近10条非置顶 Reels；均播只用 IG 原生 `ig_play_count`，总 `play_count`/`fb_play_count` 仅审计、不得抬价；默认×$35/1000，区间×$35–$40/1000；partial/fallback/missing 如实标注；不参与评分路由 |
| 实际报价 / Paid CPM | 人工+派生 | 报价必须来自红人/代理；不得用展示估价代替；实际 CPM=实际报价÷原生曝光×1000 |
