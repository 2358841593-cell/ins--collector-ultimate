...
def test_partial_fallback_and_missing_are_explicit_and_never_masquerade_as_complete():
    ...
    assert fallback["status"] == "fallback_modash"
    assert fallback["source"] == "modash_profile_fallback"
    assert fallback["sample_count"] == 0
    assert fallback["reels"] == []
    assert fallback["average_plays"] == 40_000
...
def test_delivery_status_labels_distinguish_partial_fallback_and_missing():
    partial = pricing.derive_quote_estimate(
        {
            "pricing_reel_samples": [
                _reel(f"P{i}", 10_000, i) for i in range(1, 10)
    ...
    fallback = pricing.derive_quote_estimate(
        {"avg_reels_plays": 40_000}, CFG
    )
    missing = pricing.derive_quote_estimate({}, CFG)

    assert "样本不足 9/10" in export_v2_html._pricing_cell(  # noqa: SLF001
    ...
    assert "样本不足" in export_v2_xlsx._cell(  # noqa: SLF001
        {"pricing_estimate": partial}, "pricing_status"
    )
    assert "第三方 Reels 均播替代" in export_v2_xlsx._cell(  # noqa: SLF001
...
def test_xlsx_keeps_missing_quote_numeric_columns_blank():
    ...
    assert sheet.cell(2, headers["报价状态 / 来源"]).value == "待补 Reels 播放量"
